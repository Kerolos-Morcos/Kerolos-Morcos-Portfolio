...
## Follow-up: section entrance stabilization

This supersedes the profile, timing, and normal-scroll skip behavior in the preceding notes.

- **Root cause removed:** normal entries could be converted directly to their final state by a stale-callback check, an upper-viewport-depth check, or the active-animation cap. Active effects could also be cancelled by pointer and focus events. Those competing paths produced the visible pop.
- **Current coordinator:** each static `data-motion-section` has one trigger: its first presentation group (heading where available). Below-fold groups are prepared offscreen once, then the shared observer starts the heading/content/cards/CTA groups from one timing origin, marks the coordinator complete, and unobserves it. No structural section, grid, timeline, carousel, or pagination shell is animated.
- **Normal scrolling:** no stale-age or “deep in viewport” finalization remains. The sole exceptional skip is a target already carried completely above the viewport by one exceptionally large scroll; it is restored visible rather than left prepared.
- **Profiles:** desktop is `opacity: 0 → 1`, 32px upward motion, 880ms, 80ms group steps capped at 320ms. Mobile is `opacity: 0 → 1`, 20px heading / 22px content movement, 720ms, 60ms group steps capped at 280ms. The shared easing is `cubic-bezier(.22, 1, .36, 1)`.
- **Hero:** six staged presentation groups use the same profile: badge → heading → role → description → actions → technologies. Its desktop timeline ends within 1,200ms and its mobile timeline within 1,000ms.
- **Continuous Hero motion:** desktop orbit rings are 30s/24s; mobile rings are 36s/28s in reverse directions. Pulse and ping are 4.2s desktop / 4.3s mobile; floating icons are 4.8–5.6s with a 5px maximum rise; the name sheen is 8.5s desktop / 9s mobile.
- **One-time and stability:** `WeakSet` completion records prevent reverse-scroll, subsequent downward-scroll, language, theme, or resize replay. Preparation is applied only below the initial viewport and is cleared when an entrance completes or is settled, so it cannot alter section geometry or leave a blank section. Pointer/focus cancellation has been removed.
- **Rendered checks:** clean production previews confirmed 360×800, 390×844, 430×932, 1024×800, 1440×900, and 1920×1080 profile values and prepared below-fold groups. At 1440px, a normal About entry was sampled mid-animation (heading opacity 0.81 and 5px remaining travel after 100ms); at 390px, the About coordinator produced exactly two entrance events and remained at two through a down → up → down pass. The clean desktop and mobile console logs contained no errors.
